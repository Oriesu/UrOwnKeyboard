...
def apply_source(source_type, source_id):
    ...
    if not cmd:
        return ActivationResult.fail("Fuente XKB no válida para X11.",f"{source_type}:{source_id}")
    result = run(shlex.split(cmd))
    if result.returncode != 0:
        return ActivationResult.fail("No se pudo aplicar setxkbmap.",(result.stdout + "\n" + result.stderr).strip())
...
def verify_source(source_type, source_id):
    ...
    return ActivationResult.fail("La fuente X11 aplicada no coincide.",f"esperado={wanted_layout}, actual={got}")
...
def rollback_default(layout="es"):
    ...
    if result.returncode != 0:
        return ActivationResult.fail("No se pudo restaurar layout X11.",(result.stdout + "\n" + result.stderr).strip())
